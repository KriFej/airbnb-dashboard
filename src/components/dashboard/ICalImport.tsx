...
import { Plan, canUseICal } from "@/lib/plan";
  ...
  return (
    <section className="rounded-2xl border border-border bg-white p-6">
      ...
      <p className="mt-1 text-xs text-muted">
        ...
        récupérons et fusionnons vos réservations.
      </p>
...
function SourceRow({
  label, placeholder, accent, dotColor, url, onUrl, onBookings, source, cachedCount,
}: {
  label: string; placeholder: string; accent: string; dotColor: string;
  url: string; onUrl: (u: string) => void; onBookings: (b: Booking[]) => void;
  source: Source; cachedCount: number;
}) {
  const [status, setStatus] = useState<Status>({ kind: "idle" });

  async function sync() {
    if (!url.trim()) { setStatus({ kind: "error", message: "Collez d'abord une URL." }); return; }
    setStatus({ kind: "loading" });
    try {
      const text = await fetchICal(url.trim());
      const bookings = parseICS(text, source);
      onBookings(bookings);
      setStatus({ kind: "success", count: bookings.length, at: new Date().toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" }) });
    } catch (e) {
    ...
    <div className="rounded-xl border border-border bg-white p-4">
      ...
      <div className="mt-3 flex gap-2">
        <div className="relative flex-1">
          <Link2 size={14} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-dim" />
          <input
